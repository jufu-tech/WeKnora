export default {
  ...
  integrations: {
    ...
    claw: {
      ...
      subtitle: '通过 Easy RAG REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      ecosystemNote: 'Skill 托管于 ClawHub（{\'@\'}lyingbug/easyrag），完整 API 说明与版本历史请参见 ClawHub 页面。',
      installCta: '前往 ClawHub',
      installCtaHint: '安装 Easy RAG Skill · 将在新标签页打开',
      ...
      steps: {
        ...
        env: {
          title: '配置环境变量',
          desc: '在终端或 ~/.zshrc、~/.bashrc 中设置 EASYRAG_BASE_URL 与 EASYRAG_API_KEY。下方示例已填入当前 API 地址，请将 API Key 替换为实际值。'
        },
        api: {
          title: '获取 API 凭证',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 Easy RAG 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 Easy RAG 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      capabilityManageModelsHint: '允许管理模型配置、模型凭据、模型连通性测试和 EasyRAGCloud 凭据。',
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 Easy RAG 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weasyrag、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与 Easy RAG 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
      ...
      signedRequestStep0: '# 0. 在你的后端签发 JWT（HS256，aud=easyrag，sub=用户 ID，tenant_id={tenantId}，exp 不超过 24 小时）',
  ...
  system: {
    ...
    retry: '重试',
    versionLabel: '应用版本',
    versionDescription: '当前应用服务（Easy RAG APP）的版本号',
    frontendVersionLabel: 'UI 版本',
    frontendVersionDescription: '当前 UI 界面（Easy RAG UI）的构建版本号',
  ...
  embedPublish: {
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，Easy RAG 会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
  ...
  createChat: {
    title: 'Hi，我是 Easy RAG，让你的知识触手可及',
    newSessionTitle: '新会话',
    messages: {
      createFailed: '创建会话失败',
  ...
  auth: {
    ...
    firstTime: '首次使用 Easy RAG？',
    registerSuccess: '注册成功，请登录',
    registerFailed: '注册失败',
    subtitle: 'RAG 问答、ReAct 智能体与 Wiki 知识库，大模型驱动的企业级知识框架',
    registerSubtitle: '创建账户并开始使用 Easy RAG',
  ...
  settings: {
    ...
    storage: {
      ...
      configurable: '可配置',
      pathPrefix: '路径前缀（可选）',
      pathPrefixPlaceholder: '如 Easyrag/images',
      prefixPlaceholder: '如 Easyrag',
    ...
    weknoraCloud: {
      ...
      addModelsDisplayName: {
        chat: 'EasyRAGCloud 对话',
        embedding: 'EasyRAGCloud Embedding',
        rerank: 'EasyRAGCloud ReRank',
        vllm: 'EasyRAGCloud 视觉'
      },
      modelsSection: {
        title: '云模型接入',
        descReady: '将 Easy RAG Cloud 提供的四类标准模型注册到当前空间，用于对话、向量检索、重排序与多模态理解。',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，Easy RAG 会自动解析并建立索引。点击这里进入知识库。'
      },
      welcome: {
        title: '欢迎使用 Easy RAG',
